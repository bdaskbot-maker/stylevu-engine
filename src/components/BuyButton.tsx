...
export default function BuyButton({ product, className = '' }: BuyButtonProps) {
  ...
  const handleBuy = () => {
    ...
    switch (buyLinkType) {
      case 'whatsapp': {
        ...
        const message =
          language === 'bn'
            ? `আসসালামু আলাইকুম! ${brandName} থেকে আমি "${productName}" (${product.currency || '৳'}${product.price}) অর্ডার করতে চাই। StyleVu তে পরে দেখে পছন্দ হয়েছে!`
            : `Hi! I'd like to order "${product.name}" (${product.currency || '৳'}${product.price}) from ${brandName}. I tried it on StyleVu and loved it!`;
  ...
  return (
    <button
      ...
      aria-label={`${getButtonLabel()} - ${product.name}`}
    >
